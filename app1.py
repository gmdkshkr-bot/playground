...
def get_exchange_rates():
    ...
    # KRW 대비 주요 통화의 '역' 환율 값 (1 Foreign Unit = X KRW)
    ...
    try:
        ...
        # 1. Store USD rate: 1 USD = krw_per_usd KRW
        exchange_rates['USD'] = krw_per_usd / conversion_rates.get('USD', 1.0) 
        
        # 2. Calculate EUR rate: 1 EUR = (KRW/USD) / (EUR/USD)
        eur_rate_vs_usd = conversion_rates.get('EUR', 0)
        if eur_rate_vs_usd > 0:
            exchange_rates['EUR'] = krw_per_usd / eur_rate_vs_usd 
        else:
            exchange_rates['EUR'] = FALLBACK_RATES['EUR'] # Fallback 사용

        # 3. Calculate JPY rate: 1 JPY = (KRW/USD) / (JPY/USD)
        jpy_rate_vs_usd = conversion_rates.get('JPY', 0)
        if jpy_rate_vs_usd > 0:
            exchange_rates['JPY'] = krw_per_usd / jpy_rate_vs_usd
        else:
            exchange_rates['JPY'] = FALLBACK_RATES['JPY'] # Fallback 사용
...
if 'csv_processed_flag' not in st.session_state: # 💡 New Flag
    st.session_state.csv_processed_flag = False
if 'psychological_summary' not in st.session_state:
    st.session_state.psychological_summary = pd.DataFrame() # 💡 New DF for caching chat data
...
with st.sidebar:
    ...
    if st.session_state.all_receipts_items:
        # Concatenate all dataframes to get total item count
        try:
            total_items_count = pd.concat(st.session_state.all_receipts_items, ignore_index=True).shape[0]
        except ValueError:
            total_items_count = 0
            
        st.info(f"Currently tracking {len(st.session_state.all_receipts_summary)} receipts / {total_items_count} items.") 
...
with tab1:
    ...
    def handle_csv_upload():
        if st.session_state.csv_uploader is not None:
            # 파일이 선택되었을 때만 처리 플래그를 True로 설정
            st.session_state.csv_processed_flag = True
    ...
    uploaded_csv_file = st.file_uploader(
        ...
        key='csv_uploader', 
        on_change=handle_csv_upload # 💡 on_change 콜백 사용
    )

    # 💡 로직 분리: 파일이 업로드되었고, 아직 처리되지 않았다면 처리 시작
    if st.session_state.csv_processed_flag and uploaded_csv_file is not None:
        
        # 파일 내용을 읽어 DataFrame으로 처리하는 로직
        try:
            ...
            if not all(col in imported_df.columns for col in required_cols):
                ...
                if summary_data:
                    ...
                    st.success(f"🎉 CSV 파일 **{uploaded_csv_file.name}**의 기록 (**{len(imported_df)}개 아이템**)이 성공적으로 불러와져 누적되었습니다. (총액: {summary_data['Total']:,.0f} KRW)")
        ...
        # **중요**: 처리가 완료되면 플래그를 False로 리셋합니다.
        st.session_state.csv_processed_flag = False 
        st.rerun() # 💡 상태 변경을 화면에 반영하기 위해 rerun을 사용 (Streamlit UX)
    ...
    if uploaded_file is not None:
        ...
        with col2:
            ...
            if is_already_analyzed:
                ...
                st.info(f"누적 기록 총액 (KRW): **{existing_summary.get('Total', 0):,.0f} KRW** (부가세/팁 제외된 아이템 총합)")
                st.markdown("---")
                        ...
                        try:
                            ...
                            if 'items' in receipt_data and receipt_data['items']:
                                ...
                                st.session_state.all_receipts_summary.append({
                                    ...
                                    'Total': final_total_krw, # 아이템 총합 + Tip만 더함
    ...
    **💡 Category Scheme (Sub-Category)**
    ...
    if st.session_state.all_receipts_items:
        ...
        # 💡 전체 총액 계산 및 시각적 강조
        total_spent_krw_items = all_items_df_numeric['KRW Total Spend'].sum()
        
        # Summary DF에서 세금/팁 합계
        summary_df_raw = pd.DataFrame(st.session_state.all_receipts_summary)
        total_tax_krw = summary_df_raw['Tax_KRW'].sum() if 'Tax_KRW' in summary_df_raw.columns else 0.0
        total_tip_krw = summary_df_raw['Tip_KRW'].sum() if 'Tip_KRW' in summary_df_raw.columns else 0.0
        
        # 아이템 총액 + 팁 (Tax는 품목에 포함된 경우가 많으므로 일반적으로 제외하고 팁만 더합니다.)
        final_total_spent = total_spent_krw_items + total_tip_krw 
        
        # 💡 총 지출액을 Streamlit Metric으로 표시하여 강조
        st.metric(label="💰 Total Accumulated Spending (Items + Tip)", 
                  value=f"{final_total_spent:,.0f} KRW", 
                  delta=f"+ {len(st.session_state.all_receipts_summary)} Records")
        ...
        # Tax는 품목과 분리하여 별도로 표시하는 것이 일반적이며, Tip은 사용자가 지불한 추가 비용이므로 합산
        if total_tax_krw > 0:
        ...
        if st.button("🧹 Reset Record", help="Clears all accumulated receipt analysis records in the app."):
            ...
            st.session_state.all_receipts_summary = []
            st.session_state.chat_history = [] 
            st.session_state.psychological_summary = pd.DataFrame() # 초기화
            st.rerun() 
...
with tab2:
    ...
    if not st.session_state.all_receipts_items:
        ...
        # --- 🌟 데이터 처리 및 심리 분석 (상태 관리) 🌟 ---
        all_items_df_raw = pd.concat(st.session_state.all_receipts_items, ignore_index=True)
        
        # 현재 영수증 기록을 기반으로 해시 생성
        current_data_hash = hash(tuple(item['id'] for item in st.session_state.all_receipts_summary))
        
        # 데이터가 변경되었거나, 분석 결과가 없는 경우 새로 분석하고 캐싱
        if 'last_data_hash' not in st.session_state or st.session_state.last_data_hash != current_data_hash or st.session_state.psychological_summary.empty:
            
            st.session_state.last_data_hash = current_data_hash
            
            # Defensive check for KRW Total Spend column
            if 'KRW Total Spend' not in all_items_df_raw.columns:
                 all_items_df_raw['KRW Total Spend'] = all_items_df_raw.apply(
                     lambda row: convert_to_krw(row['Total Spend'], row['Currency'], EXCHANGE_RATES), axis=1
                 )

            # 1. Add Psychological Category
            all_items_df_raw['Psychological Category'] = all_items_df_raw['AI Category'].apply(get_psychological_category)
        ...
        # 저장된 분석 결과 사용
        psychological_summary = st.session_state.psychological_summary
        all_items_df_for_chat = st.session_state.all_items_df_for_chat
        ...
        detailed_items_for_chat = all_items_df_for_chat[['Psychological Category', 'Item Name', 'KRW Total Spend']]
